...
def print_help():
    ...
    print()
    print("modes:")
    print("    single: Puts the scope in single trigger mode.")
    print("    ping: Pings the oscilloscope and prints the results. ")
    print("        Exits with 0 if response. Exits with 1 if no response")
    print(
        "    png|bmp: Grabs a screenshot of the scope display and saves it as the specified image format."
    )
    print(
        "        Exits with 0 if successful. Exits with 1 if unsuccessful (Ex: scope waiting for trigger)"
    )
    print(
        "    csv: Gets the current waveform data from the scope and saves it to a csv file."
    )
    print(
        "        Exits with 0 if successful. Exits with 1 if unsuccessful (Ex: scope waiting for trigger)"
    )
...
elif sys.argv[1].lower() not in ["png", "bmp", "csv", "single", "ping"]:
    print_help()
    print("This mode is not supported: ", sys.argv[1])
    sys.exit(1)
...
if mode == "ping":
    ...
    if response != 0:
        ...
        print("Ping response successfully received from oscilloscope.")
        sys.exit(0)
...
# if (id_fields[company] != "RIGOL TECHNOLOGIES") or (id_fields[model][:2] != "DS"):
#     print(
...
if mode in ["png", "bmp"]:
    ...
    # Save as PNG or BMP according to mode
    im = Image.open(io.BytesIO(buff))
    im.save(filename + "." + mode, mode)
    print("Saved file:", "'" + filename + "." + mode + "'")
...
# TODO: Change WAV:FORM from ASC to BYTE
    ...
    for channel in chanList:
        ...
        # MATH channel does not allow START and STOP to be set.
        if channel != "MATH":
        ...
        csv_buff_list = csv_buff.split(linesep)
        csv_rows = len(csv_buff_list)

        current_row = 0
        if csv_buff == "":
            csv_first_column = True
            csv_buff = str(channel) + linesep
        else:
            csv_first_column = False
            csv_buff = str(csv_buff_list[current_row]) + "," + str(channel) + linesep

        for point in buff_list:
            current_row += 1
            if csv_first_column:
                csv_buff += str(point) + linesep
            else:
                if current_row < csv_rows:
                    csv_buff += (
                        str(csv_buff_list[current_row]) + "," + str(point) + linesep
                    )
                else:
                    csv_buff += "," + str(point) + linesep
